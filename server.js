...
app.post('/order', (req, res) => {
   ...
   const mailOption ={
    from: 'ing.diegorodriguezv@gmail.com',
    to: email,
    ...
    <body>
        <div>
            <h1 class="heading">dear ${email.split('@')[0]},<span>su pedido se ha realizado con éxito.</span></h1>
   ...
   db.collection('order').doc(docName).set(req.body)
    .then(data => {     
         transporter.sendMail(mailOption, (err, info) => {
            if(err){
                res.json({'alert': 'opps! Parece que ocurrió algún error. Inténtalo de nuevo'})
            } else{
